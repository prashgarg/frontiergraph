...
\small
\begin{itemize}
  \item Freeze the literature at year \(t-1\).
  \item Look for absent links implied by local structure.
  \item Treat each missing directed link as a candidate next paper.
\end{itemize}
...
\begin{columns}[T]
\column{0.58\linewidth}
\includegraphics[width=\linewidth]{method_build_step2_signals.png}

\column{0.38\linewidth}
\small
\begin{itemize}
  \item \textbf{Path support}: does nearby work already point toward this link?
  \item \textbf{Gap bonus}: is the pair still underexplored directly?
  \item \textbf{Motif / hub terms}: reward plausible closure, discount pure popularity artifacts.
\end{itemize}

\vspace{0.6em}
Mainline idea: combine novelty, plausibility, and anti-hub discipline in one transparent score.
...
\includegraphics[width=\linewidth]{corpus_stock_timeline.png}

\column{0.41\linewidth}
\small
\begin{itemize}
  \item Build \(G_{t-1}\) from papers observed by year \(t-1\).
  \item Rank missing links using only information available then.
  \item Check first appearance in \([t, t+h]\), with \(h \in \{1,3,5\}\).
  \item Compare against preferential attachment and audit leakage.
\end{itemize}
...
\small
\begin{itemize}
  \item Preferential attachment is stronger in the full rolling benchmark.
  \item That mixed result is central, not a footnote.
  \item The paper's value depends on what transparent signals still add under more decision-relevant settings.
\end{itemize}
...
\small
\begin{itemize}
  \item On held-out anchors, a tuned transparent model can beat the popularity baseline.
  \item In broader top-\(K\) slices, the main model catches up and can slightly exceed preferential attachment.
  \item That makes the tool plausible as a screening layer even if popularity is hard to beat in the narrowest benchmark.
\end{itemize}
...
\small
\begin{itemize}
  \item The transparent model mostly surfaces \textbf{gap}-type questions rather than boundary-heavy popularity closures.
  \item That is exactly the kind of object we want if the paper is about research allocation rather than pure forecast mimicry.
  \item Boundary questions still matter, but they need separate evaluation because they are rarer and harder.
\end{itemize}
...
These are examples of the kind of candidate next-paper questions the system surfaces today.
\end{block}

\vspace{0.35em}
\begin{columns}[T]
\column{0.32\linewidth}
\begin{block}{Public debt and CO\(_2\) emissions}
\footnotesize
Why it may matter: debt stress can reshape investment and energy policy.

One place to start: austerity or refinancing episodes.
\end{block}

\column{0.32\linewidth}
\begin{block}{Monetary policy and energy consumption}
\footnotesize
Why it may matter: monetary shocks move spending and production, but energy demand is rarely centered.

One place to start: housing, durables, and sectoral energy use.
\end{block}

\column{0.32\linewidth}
\begin{block}{Urbanization and output growth}
\footnotesize
Why it may matter: urbanization is often backgrounded rather than tested as the growth question itself.

One place to start: regional urban expansion or city concentration.
\end{block}
\end{columns}
\end{frame}
...
\begin{tikzpicture}[x=1cm,y=1cm]
  \node[draw=DeckLine, rounded corners=6pt, fill=DeckSand, minimum width=4.8cm, minimum height=1.35cm, align=center] (core) at (2.4,3.1) {\textbf{Scientific core}\\claim-graph ranking + prospective evaluation};
  \node[draw=DeckLine, rounded corners=6pt, fill=DeckSoft, minimum width=4.2cm, minimum height=1.15cm, align=center] (site) at (1.7,1.2) {\textbf{Public site}\\research questions for browsing};
  \node[draw=DeckLine, rounded corners=6pt, fill=DeckSoft, minimum width=4.2cm, minimum height=1.15cm, align=center] (app) at (5.0,1.2) {\textbf{Workbench}\\deeper evidence and inspection};
  \draw[-{Latex[length=2.2mm]}, thick, color=DeckBlue] (core.south west) -- (site.north);
  \draw[-{Latex[length=2.2mm]}, thick, color=DeckTeal] (core.south east) -- (app.north);
\end{tikzpicture}
}
...
\small
\begin{itemize}
  \item The site and workbench are downstream artifacts.
  \item They make the research-allocation object inspectable.
  \item They are not the paper's core validation evidence.
\end{itemize}
\end{columns}
\end{frame}
