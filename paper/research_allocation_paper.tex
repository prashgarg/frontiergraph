...
\newcommand{\repoURL}{\nolinkurl{github.com/[repository-to-be-inserted-at-release]}}
...
\date{15 March 2026\\[0.3em]\normalsize Work-in-Progress Draft (Preliminary and Incomplete)\\[0.15em]\normalsize Do not cite or circulate}
...
The framework is simple. At year \(t-1\), let \(G_{t-1}=(V,E_{t-1})\) denote the claim graph assembled from papers observed through that date. A candidate next paper is represented as a missing link in that graph. For a directed causal candidate, \(u \rightarrow v\) is eligible when that ordered directed link has not yet appeared in the historical graph. For an undirected noncausal candidate, \(\{u,v\}\) is eligible when the pair has not yet appeared as undirected support. The headline object in this paper is the directed causal candidate.
...
\]
where \(\tilde P(u,v)\) is normalized path support, \(G(u,v)\) is the underexploration gap, \(\tilde M(u,v)\) is normalized motif support, and \(\tilde H(u,v)\) is the normalized hub penalty. The main specification uses \(\alpha=0.5\), \(\beta=0.2\), \(\gamma=0.3\), and \(\delta=0.2\), matching the scoring implementation used in the released outputs. Put differently, a pair scores highly when several nearby routes keep pointing toward it, the direct relation still looks oddly absent relative to that support, and the pair is not simply another generic high-degree combination. Figures~\ref{fig:gap-boundary} and~\ref{fig:score-components} are useful here because they show what the rule is actually rewarding: multiple short paths, repeated local support, and a direct link that still looks underfilled relative to its neighborhood.
...
\section{FrontierGraph as Research Tool}

\href{https://frontiergraph.com/}{FrontierGraph} is the public interface built on top of this empirical framework. The site is not the evidence for the paper's main claims, and it should not be read as a substitute for the backtest. Its role is different: it makes the ranked objects inspectable. A user can browse candidate questions, inspect supporting paths and mediators, move from a surfaced pair to nearby literature, and decide whether the object is a gap question, a boundary question, or a path-rich mechanism question that still needs substantive vetting.

The distinction matters because the research problem and the product problem are not the same. The paper is about whether a graph-based ranking can prospectively surface plausible next papers. The site is about how to make that ranking legible enough to be useful. In practice, the website solves three narrower problems. It lowers search costs by turning a large graph into a browsable list of candidate questions. It lowers interpretation costs by exposing the local supporting structure rather than only a score. And it lowers handoff costs between discovery and reading by linking surfaced questions to nearby concepts, mediators, and evidence.

That product layer also disciplines the paper. A score that cannot be explained question by question is unlikely to be useful even if it performs well in aggregate. The public interface therefore acts as a hard interpretability constraint. It pushes the project toward inspectable ranking components, plain-language candidate objects, and explanation bundles that can be audited by a researcher rather than only by the model designer. Appendix~\ref{app:tool-notes} collects the small set of product-facing diagnostics that are most useful for interpreting the paper.
...
\begin{table}[h]
  \caption{Paired bootstrap comparison: graph-based main model minus preferential attachment}
...
The website layer is designed to expose the objects the paper ranks rather than to replace the benchmark. In particular, the public interface helps with three tasks that the evaluation tables alone cannot solve: inspecting why a candidate surfaced, seeing which mediators or neighboring concepts support it, and distinguishing a local gap question from a thinner boundary question. Those are interpretation aids. They do not make the model correct.
