...
A second result changes how to read the missing-link object itself. A path-development audit suggests that the literature often deepens existing direct claims by adding mediator structure more often than it closes a missing direct link already implied by local paths. So the graph is useful not only for surfacing local gaps, but also for showing when a topic may be more likely to evolve through mechanism-building than through direct closure.
