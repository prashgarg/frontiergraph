...
PAPER_OUT_DIR = ROOT / "outputs" / "paper" / "47_ontology_grounding_sensitivity_reviewed_round2"
...
GROUNDING_REVIEWED_PATH = DATA_DIR / "extraction_label_grounding_v2_reviewed_round2.parquet"
OVERLAY_REVIEWED_PATH = DATA_DIR / "ontology_enrichment_overlay_v2_reviewed_round2.parquet"
PROPOSALS_REVIEWED_PATH = DATA_DIR / "ontology_missing_concept_proposals_v2_reviewed_round2.parquet"
CLUSTERS_REVIEWED_PATH = DATA_DIR / "unresolved_label_cluster_summaries_v2_reviewed_round2.parquet"
NOTE_PATH = DATA_DIR / "reviewed_overlay_application_note_round2.md"
...
def normalize_remaining_majority() -> pd.DataFrame:
    majority = pd.read_parquet(REMAINING_ROW_MAJORITY_PATH)
...
def build_review_layers() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, object]:
    reviewed = load_module("build_reviewed_ontology_overlay_v2.py", "overlay_reviewed_base_round2")
    ...
    majority_row_reviews = remaining_majority.drop_duplicates(subset=["label"]).set_index("label")
    combined_row_reviews = pd.concat(
        [original_row_reviews[~original_row_reviews.index.isin(majority_row_reviews.index)], majority_row_reviews],
        axis=0,
    )
    cluster_reviews = (
    ...
    for _, row in audit.iterrows():
        ...
        if row_review is not None:
            ...
            if label in majority_row_reviews.index:
                decision_source = "remaining_row_majority_review"
            else:
...
def main() -> None:
    ...
    print("Round-2 reviewed overlay artifacts written.")
