...
DEFAULT_OUTPUT_ROOT = "data/production/frontiergraph_concept_beta"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build concept-level graph, corpus, and app DB from ontology v2.")
...
def derive_stability_row(row: pd.Series) -> float:
    claim_base = {
        ...
        "question_only": 0.22,
        "other": 0.4,
    }.get(str(row.get("claim_status", "")), 0.4)
    explicitness_adj = {
        "result_only": 0.08,
...
def load_concept_level_frames(extraction_db: Path, ontology_db: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    try:
        ...
        mappings = pd.read_sql_query(
            """
            SELECT im.custom_id, im.node_id, im.normalized_label, im.concept_id, im.mapping_source, im.confidence,
            ...
            LEFT JOIN head_concepts hc ON hc.concept_id = im.concept_id
            """,
            onto,
        )
...
def build_concept_graph_outputs(
    concept_nodes: pd.DataFrame,
    concept_edge_instances: pd.DataFrame,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    concept_edges = (
        ...
        .rename(columns={"source_concept_id": "source_concept_id", "target_concept_id": "target_concept_id"})
    )
...
def build_concept_app_db(
    ...
    corpus_path = artifact_root / "concept_corpus.parquet"
    pairs_path = artifact_root / "concept_pairs.parquet"
    paths_path = artifact_root / "concept_paths.parquet"
    motifs_path = artifact_root / "concept_motifs.parquet"
    candidates_path = artifact_root / "concept_candidates.parquet"
    app_db_path = output_root / "concept_beta_app.sqlite"
    ...
    try:
        ...
        explanation_tables["candidate_supporting_papers"].to_sql(
            "candidate_supporting_papers", conn, if_exists="replace", index=False
        )
        ...
        pd.DataFrame(
            [
                {"key": "app_mode", "value": "concept_beta"},
                {"key": "ontology_version", "value": "v2"},
                {"key": "graph_view", "value": "concept"},
            ]
...
def main() -> None:
    ...
    works, nodes, edges, mappings, contexts = load_concept_level_frames(
    ...
    concept_edges, concept_edge_profiles, concept_edge_contexts, concept_edge_exemplars = build_concept_graph_outputs(
        concept_nodes=concept_nodes,
        concept_edge_instances=concept_edge_instances,
    )

    graph_db_path = output_root / "concept_graph.sqlite"
